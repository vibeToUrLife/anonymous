    ...
    function drawDecorPreview(cvs, decorId, category) {
      ...
      const furnArt = typeof furnitureArt === 'function' ? furnitureArt(decorId) : null;
      if (furnArt) {
        // Floor furniture is artwork in the room, so the card shows that very
        // picture — a shop that promises one thing and delivers another is a
        // shop nobody trusts twice. Stood on the card's floor line, not centred.
        if (furnArt.naturalWidth) {
          const maxW = w - 10, maxH = h - 6;
          let iw = maxW, ih = iw * furnArt.naturalHeight / furnArt.naturalWidth;
          if (ih > maxH) { ih = maxH; iw = ih * furnArt.naturalWidth / furnArt.naturalHeight; }
          ctx.drawImage(furnArt, cx - iw / 2, h - 3 - ih, iw, ih);
        } else {
          // Still downloading — repaint this card once it lands.
          furnArt.addEventListener('load', () => drawDecorPreview(cvs, decorId, category), { once: true });
        }
      } else if (decorId === 'clock') {
        const cr = 14;
